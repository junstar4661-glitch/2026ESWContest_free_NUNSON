...
| 방향 | 토픽 | 타입 |
| --- | --- | --- |
| 팔 → 상위 제어부 | `/arm_status` | ArmStatus |
| 팔 → 상위 제어부 | `/detected_objects` | DetectedObjectArray |
| 상위 제어부 → 팔 | `/chassis_mode` | ChassisMode |
| 상위 제어부 → 팔 | `/arrival_status` | ArrivalStatus |
...
⚠️ **이 모듈은 ROS 비의존(순수 파이썬)으로 유지한다.** 상위 제어부 contract.py 와 1:1로
   대조할 수 있어야 하고, ROS 없이 테스트할 수 있어야 한다.
   QoS 프로파일은 rclpy 가 필요하므로 `qos_profiles.py` 로 분리했다.
"""

# ── 상위 제어부 → 팔 : ArrivalStatus.status ──
ARRIVED_PICKUP = 'ARRIVED_PICKUP'
ARRIVED_DROP = 'ARRIVED_DROP'

# ── 상위 제어부 → 팔 : ChassisMode.mode ──
...
# ── 팔 → 상위 제어부 : ArmStatus.status (계약 v1 어휘) ──
ARM_IDLE = 'IDLE'
